...
func TestMiddleware(t *testing.T) {
	...
	middleware3 := func(next MiddlewareFunc) MiddlewareFunc {
		return func(ctx context.Context, req interface{}) (resp interface{}, err error) {
			fmt.Printf("middleware 3 start\n")

			resp, err = next(ctx, req)
			if err != nil {
				return
			}
			fmt.Printf("middleware3 end\n")
			return
		}
	}
	...
	chain := Chain(outer, middleware1, middleware2, middleware3)
